#!/usr/bin/env bash
#
# Execute the blockchain-module doc-test end-to-end and regenerate its Markdown.
#
# There is one spec:
#   blockchain-module-runtime.test.yaml — packages this module as an .lgx,
#       installs it with lgpm, and drives it through a headless logoscore daemon.
#
...
# published flake. The spec pins `github:logos-blockchain/logos-blockchain-module{release}`
...
# to logos-blockchain/logos-blockchain-module. A local-only / uncommitted HEAD
# won't resolve; export COMMIT="" (or push first) in that case.
COMMIT="${COMMIT-$(git rev-parse HEAD)}"
RELEASE_FOR=()
if [ -n "${COMMIT}" ]; then
  RELEASE_FOR=(--release-for "logos-blockchain-module=${COMMIT}")
  echo "==> Pinning logos-blockchain-module to ${COMMIT}"
else
  echo "==> COMMIT empty; building from latest logos-blockchain-module master"
fi
...
# passing it once per spec keeps each spec's artifacts beside its generated .md.
